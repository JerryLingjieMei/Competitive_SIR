...
class SirNaive(object):
    ...
    def __init__(self, graph: ig.Graph, infection_rate=0.002, recover_rate=0.01,
        ...
        self.recovered = set()
        self.susceptible = set(self.graph.vs.indices).difference_update(self.infected)
...
class SirReedFrost(object):
    ...
    def __init__(self, graph: ig.Graph, time_step=15, transmission_rate=0.02, verbose=True, plot=True):
        self.graph = graph
        self.time_step = time_step
        self.transmission_rate = transmission_rate
        self.infection_rate = 1 - (1 - transmission_rate) ** (1 / time_step)
        self.verbose = verbose
        self.plot = plot
        self.infected = set(random.sample(list(self.graph.vs.indices), k=1))
        self.recovered = set()
    ...
    def run(self):
        ...
        recover_time = [0] * len(self.graph.vs.indices)
        for vertex in self.infected:
            recover_time[vertex] = self.time + self.time_step
        while len(self.infected) > 0:
            self.time += 1
            infected_ls = []
            for vertex in self.susceptible:
                self.infection_event(vertex, infected_ls, recover_time)
            self.infected.update(infected_ls)
            self.susceptible.difference_update(infected_ls)
            if self.verbose:
            ...
            self.infected.difference_update(recover_ls)
            self.recovered.update(recover_ls)
            if self.verbose:
    ...
    def infection_event(self, vertex, infected_ls, recover_time):
        ...
        if random.random() > (1 - self.infection_rate) ** infected_neighbors:
            infected_ls.append(vertex)
            recover_time[vertex] = self.time + self.time_step
